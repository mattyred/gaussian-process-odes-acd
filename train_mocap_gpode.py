...
from src.gpode.mocap_model_builder import build_model
from src.gpode.mocap_model_builder import compute_loss
from src.gpode.mocap_model_builder import compute_predictions
from src.gpode.mocap_model_builder import compute_test_predictions, compute_summary
from src.gpode.mocap_model_builder import compute_inducing_variables_for_plotting
from src.gpode.mocap_initialization import initialize_inducing, initialize_latents_with_data, \
    initialize_noisevar, initialize_and_fix_kernel_parameters
from src.gpode.plots_mocap import plot_pca_predictions, plot_data_predictions, plot_inducing_posterior_3d
from src.gpode.plots_mocap import plot_trace
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        predicted_zs = compute_predictions(model, train_ts, eval_sample_size=args.eval_sample_size)
        predicted_ys = torch.stack([latent2data_projection(p) for p in predicted_zs])
    model = initialize_noisevar(model, 1.5 * torch2numpy((train_ys_full - predicted_ys).var((0, 1, 2)) + 1e-4))
    ...
    for itr in range(1, args.num_iter):
        try:
            ...
            loss, observ_nll, init_kl, inducing_kl = compute_loss(model, train_ys_full, train_ts)
